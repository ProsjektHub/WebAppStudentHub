...
function updateTotal() {
  ...
  const expenseTotal = transactions
    ...
    const balanceTotal = incomeTotal - Math.abs(expenseTotal);

  // Update balanceElement style colors
balanceElement.style.backgroundColor = balanceTotal === 0 ? 'lightblue' : balanceTotal > 0 ? 'lightgreen' : 'lightcoral';
...
function renderList() {
  ...
  transactions.forEach(({ id, type, name, amount, date, category }) => {
...
$(document).ready(function() {
    $("#transactionForm").submit(function(event) {
        ...
        var type = $("input[name='type']").is(":checked") ? "income" : "expense";
...
console.log("Category: " + category);
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $("#status").html('<p class="success-message">' + response + '</p>');
            ...
            error: function(xhr, status, error) {
                ...
                $("#status").html('<p class="error-message">An error occurred. Please try again later.</p>');
            }
        });
    });
});
